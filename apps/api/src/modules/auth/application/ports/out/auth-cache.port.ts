...
/**
 * 인증 캐시 저장소 포트
 * JWT 토큰, 인증 코드, 검증 데이터 및 소셜 로그인 임시 데이터 관리를 위한 인터페이스입니다.
 */
export abstract class AuthCachePort {
   ...
   * @param expiresIn 만료 시간(초)
   */
  abstract saveVerificationData(verificationId: string, data: object, expiresIn: number): Promise<void>
   ...
   */
  abstract getVerificationData<T = object>(verificationId: string): Promise<T | null>

  /**
   * 검증 데이터 삭제
   * @param verificationId 검증 ID
   */
  abstract deleteVerificationData(verificationId: string): Promise<void>
