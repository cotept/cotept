...
export abstract class TokenStoragePort {
   ...
   * @param expiresIn 만료 시간(초)
   */
  abstract saveRefreshTokenFamily(
    userId: string,
    familyId: string,
    tokenId: string,
    expiresIn: number
   ...
   */
  abstract getRefreshTokenFamily(userId: string, familyId: string): Promise<string | null>;
   ...
   */
  abstract getUserIdByAuthCode(code: string): Promise<string | null>;
