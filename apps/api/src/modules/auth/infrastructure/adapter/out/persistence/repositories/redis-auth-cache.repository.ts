...
/**
 * Redis를 사용한 인증 캐시 저장소 구현
 */
@Injectable()
export class RedisAuthCacheRepository implements AuthCachePort {
  private readonly logger = new Logger(RedisAuthCacheRepository.name)

  // 인증 관련 Redis 키 접두사 (CacheService에서 네임스페이스 자동 처리)
   ...
   * @param data 검증 데이터 객체
   * @param expiresIn 만료 시간(초)
   */
  async saveVerificationData(verificationId: string, data: object, expiresIn: number): Promise<void> {
    try {
      const key = `${this.VERIFICATION_PREFIX}${verificationId}`
      await this.redisService.setObject(key, data, expiresIn)
      this.logger.debug(`Verification data saved for ID ${verificationId} with expiration ${expiresIn}s`)
    } catch (error) {
      this.logger.error(
        `Failed to save verification data: ${ErrorUtils.getErrorMessage(error)}`,
        ErrorUtils.getErrorStack(error),
      )
      throw error
    }
   ...
   * 검증 데이터 조회
   * @param verificationId 검증 ID
   * @returns 검증 데이터 또는 null
   */
  async getVerificationData<T = object>(verificationId: string): Promise<T | null> {
    try {
      const key = `${this.VERIFICATION_PREFIX}${verificationId}`
      const data = await this.redisService.getObject<T>(key)
      this.logger.debug(`Verification data lookup result for ${key}: ${data ? "found" : "not found"}`)
      return data
    } catch (error) {
      this.logger.error(
        `Failed to get verification data: ${ErrorUtils.getErrorMessage(error)}`,
        ErrorUtils.getErrorStack(error),
      )
      throw error
    }
   ...
   * @param verificationId 검증 ID
   */
  async deleteVerificationData(verificationId: string): Promise<void> {
    try {
      const key = `${this.VERIFICATION_PREFIX}${verificationId}`
      await this.redisService.delete(key)
      this.logger.debug(`Verification data deleted: ${key}`)
    } catch (error) {
      this.logger.error(
        `Failed to delete verification data: ${ErrorUtils.getErrorMessage(error)}`,
        ErrorUtils.getErrorStack(error),
      )
      throw error
    }
