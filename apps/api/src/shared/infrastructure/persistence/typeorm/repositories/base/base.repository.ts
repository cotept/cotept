...
export class BaseRepository<T extends BaseEntity<T>> extends AbstractRepository<T> {
  ...
  async executeTransactionWithRetry<R>(
    ...
    this.logger.debug(`[${this.serviceName}] Starting transaction with retry (max: ${maxRetries}, isolation: ${isolationLevel})`);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (err.code === '40001' && attempt < maxRetries) {
          ...
          this.logger.warn(`[${this.serviceName}] Transaction serialization failure, retrying in ${delay}ms (attempt ${attempt}/${maxRetries})`);
