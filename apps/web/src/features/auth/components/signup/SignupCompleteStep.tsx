    ...
    return (
      <>
        <StatusMessage
          ...
          icon={<PartyPopper className="text-success h-4 w-4" />}>
          <p className="text-success mt-2 text-sm">로그인 후 코테PT의 모든 기능을 경험해 보세요!</p>
        </StatusMessage>
        ...
        <StatusMessage variant="error" message={errorMessage?.message || "회원가입 처리 중 오류가 발생했습니다."}>
          {errorMessage?.messageDetails && (
            <div className="mt-2 flex justify-center">
              <ul className="text-destructive list-decimal text-left text-sm">
