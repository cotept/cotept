...
// dismiss(외부 클릭, StrictMode 언마운트 등) 시 동작 제어
const confirmedOrDismissed = await overlay.openAsync<boolean>(
  ({ isOpen, close, ...props }) => (
    <SystemConfirmDialog
      {...props}
      isOpen={isOpen}
      close={close}
      title="멘토 전환"
      description="지금 바로 멘토 프로필을 작성할까요?"
    />
  ),
  {
    rejectOnDismiss: false, // dismiss 시 에러 대신 false 반환
    dismissValue: false,
    onDismiss: () => console.info("confirm dialog dismissed"),
  },
)
```
...
### 3. 선언형 헬퍼 (`openOverlay`, `createOverlayHelper`)

JSX 없이 오버레이를 Promise API로 사용할 때는 `openOverlay`를 이용합니다.
...
반복적으로 쓰는 경우 `createOverlayHelper`로 래핑합니다.
...
const confirmed = await confirmDelete(
  ...
  { rejectOnDismiss: false, dismissValue: false },
)
```
