...
# Coturn 서버를 위한 보안 그룹
resource "oci_core_network_security_group" "coturn" {
  compartment_id = var.compartment_id
  vcn_id         = oci_core_vcn.main.id
  display_name   = "${var.project_name}-${var.environment}-coturn-sg"

  freeform_tags = var.additional_tags
}

# STUN/TURN TCP 규칙
resource "oci_core_network_security_group_security_rule" "coturn_stun_turn_tcp" {
  network_security_group_id = oci_core_network_security_group.coturn.id
  direction                 = "INGRESS"
  protocol                  = "6" # TCP
...
# STUN/TURN UDP 규칙
resource "oci_core_network_security_group_security_rule" "coturn_stun_turn_udp" {
  network_security_group_id = oci_core_network_security_group.coturn.id
  direction                 = "INGRESS"
  protocol                  = "17" # UDP
...
# 미디어 트래픽을 위한 UDP 포트 범위 규칙
resource "oci_core_network_security_group_security_rule" "coturn_media_udp" {
  network_security_group_id = oci_core_network_security_group.coturn.id
  direction                 = "INGRESS"
  protocol                  = "17" # UDP
  ...
  udp_options {
    destination_port_range {
      min = var.allowed_webrtc_ports.min
      max = var.allowed_webrtc_ports.max
    }
  }
}
